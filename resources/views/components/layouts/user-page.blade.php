...
<head>
    ...
    <style>
        .progress-slide-carousel .swiper-pagination-progressbar .swiper-pagination-progressbar-fill {
            background-color: rgb(230, 23, 23) !important;
        }
    </style>
    @livewireStyles
    @livewireScripts
        ...
        <div class="max-w-full flex flex-wrap items-center justify-between mx-auto px-5 py-2">
            ...
            <div class="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="navbar-user">
                <ul
                    ...
                    <li>
                        <a href="{{ route('home.user') }}" wire:navigate
    ...
    <aside id="cta-button-sidebar" class="fixed top-0 left-0 z-40 w-64 h-screen mt-14 transition-transform -translate-x-full sm:translate-x-0" aria-label="Sidebar">
        <div class="h-full px-3 py-4 overflow-y-auto bg-gray-50 dark:bg-gray-800">
            <ul class="space-y-2 font-medium">
                <li>
                    <a href="{{ route('home.user') }}" wire:navigate class="flex items-center p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 group
                    {{ request()->routeIs('home.user') ? 'text-red-800 dark:text-red-800' : 'text-gray-900 dark:text-white' }}">
                ...
                <li x-data="{ open: '{{ request()->routeIs('product.*') }}' }">
                    ...
                    <ul x-show="open" class="py-2 space-y-2">
                        <li>
                            <a href="{{ route('product.women')}}" wire:navigate 
                                class="flex items-center w-full p-2 rounded-lg pl-11 group hover:bg-gray-100 dark:hover:bg-gray-700
                                {{ request()->routeIs('product.women') ? 'text-red-800 dark:text-red-800' : 'text-gray-900 dark:text-white' }}">
                                <i class="fa-solid fa-xl fa-person-dress mr-2"></i>Woman Product
                            </a>
                        </li>
                        <li>
                            <a href="{{ route('product.men') }}" wire:navigate 
                                class="flex items-center w-full p-2 rounded-lg pl-11 group hover:bg-gray-100 dark:hover:bg-gray-700
                                {{ request()->routeIs('product.men') ? 'text-red-800 dark:text-red-800' : 'text-gray-900 dark:text-white' }}">
                ...
                <li>
                    <a href="{{ route('brand.index') }}" class="flex items-center p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 group
                        {{ request()->routeIs('brand.index') ? 'text-red-800' : 'text-gray-900  dark:text-white' }}">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-6 {{ request()->routeIs('brand.index') ? 'text-red-800' : 'text-black dark:text-white' }}">
            ...
            <div id="dropdown-cta" class="p-4 mt-6 rounded-lg bg-blue-50 dark:bg-blue-900" role="alert">
                ...
                <p class="mb-3 text-sm text-blue-800 dark:text-blue-400">
                    Special for Today Promo 50% all Products for you first order, Go check it out</p>
            </div>
        </div>
    </aside>
